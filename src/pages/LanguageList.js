...
import updateToLanguage from "../store/actions/languageActions";
...
export default function LanguageList({ edit }) {
  ...
  const handleUpdateToLanguage = (language) => {
    dispatch(updateToLanguage(language));
  };

  const handleEditLanguage=(status)=>{
  ...
  useEffect(() => {
    ...
    languageService
      .getByCurriculumVitae(id)
      .then((result) => setLanguages(result.data.data));
  }, []);
  return (
    <div>
      <Card color="orange" fluid className="cvlanguagecard">
        <Card.Content
          header="Yabancı Dil Deneyimleri"
          className="cvheadertittle"
        />
        {languages.map((language) => (
          <Grid className="cvlanguagecardgrid">
            <Grid.Row>
              <Grid.Column width="1">
                <Card.Content>
                  {/* <svg
              ...
              <Grid.Column width="15">
                <Grid columns="equal" className="cvlanguageequalgrid">
                  <Grid.Row>
                    <Grid.Column width="5">
                      <Card.Content>
                ...
                <Card.Content extra>
          <Button primary onClick={()=>{handleUpdateToLanguage(language); handleEditLanguage(false)}} floated="right">
            Düzenle
          </Button>
